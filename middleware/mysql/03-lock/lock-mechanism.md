...
**意向锁不会阻塞行锁**：IS/IX 只与表级 S/X 锁有冲突关系，与行级 S/X 锁完全不冲突。意向锁的唯一用途是让"加表锁"的操作能快速判断是否有行锁存在，不参与行级并发控制。
...
**退化规则总结**：RR 下各种查询的加锁会退化——唯一索引等值命中退化 Record（去 Gap），唯一索引等值未命中退化 Gap（去 Record），范围查询不退化（Next-Key Lock）。退化的本质是"唯一性保证"——唯一索引确保不会有重复值，无需额外锁间隙防插入；非唯一索引有重复值，必须锁间隙。
...
**追问：乐观锁重试次数怎么设？** 通常 3 次，指数退避（10ms/50ms/200ms）。若 3 次仍失败说明冲突极严重，应降级为悲观锁或拒绝请求（限流）。重试需保证业务方法幂等——否则重试会导致重复执行（如重复扣款）。
