...
> **一句话定位**：索引是 MySQL 面试的起手题，"讲讲索引底层结构"几乎每场必问，能讲到 B+树页结构与千万行推导才算合格。
> **面试热度**：⭐⭐⭐⭐⭐
...
**与 B 树的本质差异**：B 树每个节点都存数据行，单页 16KB 放了数据后只能放几十个键值，扇出小、树高大（10+ 层存百万行）；且 B 树范围查询需要中序遍历回溯到父节点，无法像 B+树那样沿叶子链表线性扫描。InnoDB 自始采用 B+树（而非 B 树），正是基于 IO 次数与缓存命中率的双重优势。
...
flowchart TD
    ...
    C --> D["可存行数 = 1,000,000 × 16 = 16,000,000<br/>约 1600 万行（行越小行数越多，1KB 行约 1600 万、100B 行可达亿级）"]
```
...
**页合并（Page Merge）**：DELETE 后页内空间利用率低于 `MERGE_THRESHOLD`（默认 50%）时，InnoDB 会尝试与相邻页合并，释放空页。频繁删除+插入的表若页分裂与合并反复发生，会产生碎片——可用 `OPTIMIZE TABLE` 或 `ALTER TABLE t ENGINE=InnoDB` 重建表回收碎片。`MERGE_THRESHOLD` 可在建索引时通过 `COMMENT` 指定：`CREATE INDEX idx ON t(col) COMMENT 'MERGE_THRESHOLD=40';`。
...
**MRR 工作机制**：①Server 层向 InnoDB 请求二级索引范围扫描；②InnoDB 在二级索引树上扫描，把命中的 `(索引列值, 主键值)` 对放入 `read_rnd_buffer_size`（默认 256KB）；③缓冲区满或扫描结束，对主键值排序；④按排序后的主键顺序批量回表聚簇索引，IO 模式从随机转为顺序；⑤对 Buffer Pool 更友好——相邻主键的页大概率在同一区（extent），预读效率高。
...
- **事务内统计信息陈旧**：`@Transactional` 开启后，事务内大量 INSERT/UPDATE 不会立即反映到 `mysql.innodb_index_stats`/`innodb_table_stats`——持久化统计信息由后台 `dict_stats` 线程异步更新（默认采样 20 页），事务内写入尚未被采样刷新，优化器基于旧统计信息选索引可能选错。
...
**典型坑：`@Transactional` 包裹大批量导入导致索引选择劣化**：批量 INSERT 1 万行后立即查询，事务内统计信息未更新（持久化统计信息由后台 `dict_stats` 线程异步采样，事务内写入尚未刷新），优化器基于旧统计信息选错索引。解法：①把查询拆到事务外；②或显式 `ANALYZE TABLE` 后再查；③或用 `FORCE INDEX` 临时规避。这是"事务内统计信息陈旧"对索引选择的实际影响。
