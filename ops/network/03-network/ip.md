...
**方案对比：/16 vs /12 Pod CIDR**

| 维度 | /16 Pod CIDR | /12 Pod CIDR |
|------|-------------|-------------|
| 容量 | 65536 IP | 1677 万 IP |
| 可支持节点（每节点 /24） | 256 节点 | 65536 节点 |
| 适用规模 | 小型集群（< 256 节点） | 中大型集群（500-1000 节点） |
| 跨集群冲突 | 易冲突（多集群都占 10.244.0.0/16） | 可分配不同 /12 段（10.224.0.0/12 vs 10.240.0.0/12） |
| 路由表大小 | 256 条节点路由 | 65536 条节点路由（需 BGP 或等价路由） |
...
flowchart TD
    ...
    A --> C[Pod CIDR 10.244.0.0/16<br/>Calico/Ipallo CIDR]
...
| Pod CIDR | 10.244.0.0/16 | 65536 | 每节点 /24，500 节点 × 256 = 12.8 万 IP |
| Service CIDR | 10.96.0.0/16 | 65534 | ClusterIP，5000 服务绰绰有余 |
| 多集群预留 | 10.128.0.0/9 | 33M | 集群 2 用 10.224.0.0/12，集群 N 用 10.240.0.0/12 |

**关键设计决策**：

1. **Pod CIDR 用 /16 而非 /12**：500 节点用 /16（256 子网 × 256 IP = 65536）足够，路由表 256 条节点路由可控。若用 /12 需 BGP 或等价多路径路由，运维复杂度上升。规模超 1000 节点才考虑 /12。
...
5. **跨集群 Pod CIDR 隔离**：多集群互不重叠，集群 1 用 10.244.0.0/16，集群 2 用 10.245.0.0/16，避免 Pod 跨集群路由冲突。跨集群通信走 Ingress/Egress Gateway 或 Submariner 做路由通告。
...
--cluster-cidr=10.244.0.0/16,fd00:10:244::/56      # IPv4 + IPv6 Pod CIDR
...
> **面试加分点**：①强调 Pod CIDR 与 Service CIDR 必须不重叠；②讲清 /16 vs /12 的容量与路由表大小权衡；③NodePort 默认范围不够需调整；④IPv4 耗尽用 IPv6 双栈是云原生趋势，K8s 1.20+ 原生支持；⑤多集群 CIDR 隔离避免路由冲突。
