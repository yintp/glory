...
**为什么硬链接不能链接目录**：POSIX 默认禁止硬链接到目录，防止目录树出现环（`a/b -> a` 会导致路径解析死循环）。Linux 内核默认对所有用户（含 root）禁用此能力——历史上 root 可通过 `link()`/`linkat()` 系统调用强制创建（用于实现 `rename` 的原子操作），现代发行版通过 `fs.protected_hardlinks` 等 sysctl 进一步收紧非特权用户。**注意区分**：`fs.protected_hardlinks` sysctl 是安全限制，禁止非特权用户硬链接到无权访问的文件（防越权提权），与"硬链接不能链接目录"是两件事。软链接链接目录是允许的（但要小心循环）。
