...
| swappiness | 内核回收匿名页 vs 文件页的倾向度（0-200（5.8+，旧为 0-100），默认 60） | 回收 |
...
当物理内存紧张，内核通过 `kswapd`（`mm/vmscan.c`）后台回收页面：①**文件页**——干净页直接丢弃（下次访问重新从磁盘读），脏页先 writeback 再丢弃；②**匿名页**——写入 swap 区（磁盘分区或 zram），下次访问触发 major fault 读回。`vm.swappiness`（范围 0-200（5.8+，旧为 0-100），默认 60）控制**回收匿名页的倾向**：值 0 表示尽量不回收匿名页（仅当内存与 swap 都将耗尽才用）；100 表示匿名页与文件页等权回收；200 表示激进回收匿名页。
